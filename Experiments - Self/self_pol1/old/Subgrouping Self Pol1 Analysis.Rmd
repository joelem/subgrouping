---
title: "Subgrouping Analysis Self Pol1 Analysis"
...
```{r, echo=FALSE}
files = Sys.glob('data/sg_self1_responses/*.csv')
...
# remove outlier in 100% condition, SubjID 90513253 
#data = subset(data, SubjID!=90513253) 
```
...
ggplot(learning_avg1, aes(x = opinion_round, y = Accuracy, color = as.factor(Deviant_threshold), 
  ...
  facet_grid(.~party)
#ggsave("opinionlearningparty.png", width = 6.5, height = 5, dpi = 500)
...
ggplot(sim_sum1, aes(x = pairings, y = avg))+
  ...
  geom_point(data=similarity_summary, aes(y=Mean, color=pairings), size = 3, position = position_nudge(x=.25))+
  geom_errorbar(data=similarity_summary, aes(y=Mean, ymax=H.CI, ymin=L.CI, color=pairings), width=.2, position = position_nudge(x=.25))
#ggsave("similarityparty.png", width = 7.5, height = 6, dpi = 500)
```
...
#ggsave('identificationparty.png', width = 7.5, height = 6)
```
...
#ggsave('id_to_stim.png', width = 6.5, height = 5, dpi = 500)
...
#party_data2<-subset(party_data, !(Deviant_threshold == 1 & diff == 59 ))  # outlier in 100% condition, SubjID 90513253
