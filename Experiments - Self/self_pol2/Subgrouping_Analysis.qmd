...
ggsave('figs/exp4-learning.eps', device = cairo_ps, height = 2.5, width = 5)
```
...
similarity = data %>% 
  ...
  mutate(targetpair = factor(targetpair, levels = c('Mismatch', 'Match')))
...
ggsave('figs/exp4-similarity.eps', device = cairo_ps, height = 2.5, width = 4.5)
```
...
belowmod = similarity_sub %>% 
  filter(Deviant_threshold <= .5 & targetpair == 'Match') %>%
...
ggsave('figs/exp4-kcluster.eps', device = cairo_ps, height = 2.5, width = 2.5)
```
...
ggsave('figs/exp4-confidence.eps', device = cairo_ps, height = 2.5, width = 2.5)
```
