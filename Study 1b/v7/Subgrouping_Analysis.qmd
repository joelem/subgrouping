...
data = data %>% 
  ...
  #does self-polparty match the agents' polparty?
  mutate(same_party = case_when(polparty == 'Democratic' &
                                  Condition == 'Democrat' ~ "Ingroup",
                                polparty == 'Republican' &
                                  Condition == 'Republican' ~ "Ingroup",
                                .default = 'Outgroup'))
...
ggsave('figs/exp2-learning.eps', device = cairo_ps, height = 2.5, width = 5)
```
...
ggsave('figs/exp2-similarity.eps', device = cairo_ps, height = 2.5, width = 4.5)
```
...
ggsave('figs/exp2-kcluster.eps', device = cairo_ps, height = 2.5, width = 2.5)
```
...
ggsave('figs/exp2-confidence.eps', device = cairo_ps, height = 2.5, width = 2.5)
```
...
### Moderator Last Opinion {.tabset title="Moderator: Ingroup"}

```{r}
#| title: Table

table1(~ as.factor(Deviant_threshold)| polparty+same_party , predictions)
```

```{r}
#| title: Plot

predictions_ig_avg = predictions %>%
  group_by(Deviant_threshold, same_party) %>%
  summarize(n = n(),
            m.conf = mean(confidence),
            ci = qt(0.975,df=n-1)*sd(confidence)/sqrt(n),
            l.ci= m.conf-ci,
...
(predictions %>%
    ...
    geom_point(alpha = .2,
             size = .7)+
    geom_point(data = predictions_ig_avg,
               aes(y = m.conf),
               color = 'darkblue',
               position = position_nudge(-.15),
               shape = 5)+
    geom_errorbar(data = predictions_ig_avg,
                  aes(y = m.conf,
                      ymax=h.ci,
                      ymin=l.ci),
                  color = 'darkblue',
  ...
  scale_y_continuous(labels = seq(0,100,10),
    ...
    scale_x_discrete(labels = seq(0,1,.25),
                     breaks = seq(0,1,.25))+
    facet_grid(~same_party)+
    ggtitle("Ingroup vs Outgroup")) %>%
  ggplotly()
```
...
belowmod.extra = predictions %>%
  ...
  mutate(id = 'below_.5') %>%
  select(same_party, id, r.squared, adj.r.squared, df, df.residual, nobs)
...
bind_rows(belowmod, abovemod) %>%
  arrange(same_party, desc(id)) %>%
  relocate(same_party, id) %>%
  na.omit()
```
...
predictions %>%
  ...
  lm(confidence ~ deviance*same_party, .) %>%
  anova()
```
...
- The moderator for the prediction of the new agent's choice is operationalized as whether the prediction matched the majority opinion in the last opinion round
