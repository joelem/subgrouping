...
```{r}
#| title: New Agent Prediction Model

# https://datacolada.org/62
#two line test
breakpoint = .5

predictions %>% 
  mutate(breakpt = ifelse(Deviant_threshold < breakpoint,
                          paste0("<", breakpoint),
                          paste0(breakpoint, "<="))) %>% 
  group_by(breakpt) %>% 
  do(tidy(lm(confidence ~ Deviant_threshold, .))) %>% 
