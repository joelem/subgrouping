...
Note: This code produces a Quarto dashboard that comprehensively shows the results for an experiment from an in progress project.
...
Hypothesis: We predict that as a new agent’s deviance from the group stereotype increases there will be a transition from group updating to subgroup formation to subtype formation. This will be reflected in participants’ similarity-rating derived cluster metrics: ISM cluster analysis and participant's predictions of a new agent's behavior.
...
#set analysis options to help run mixed models
...
## Learning Analysis- Did participants learn the agents?
...
#Run mixed model for accuracy in learning performance
...
## Similarity analysis - Did participants identify the deviant?
...
#Process and code the agent pairs
similarity = data %>% 
...
#mixed model for similarity ratings
similaritymod = similarity %>% 
  lmer(response ~ targetpair*Deviant_threshold + (targetpair|SubjID),
       REML = F,
...
#extract estimates and comparisons from the model
summary(emtrends(similaritymod, pairwise ~ targetpair, var = 'Deviant_threshold', infer= c(T,T)))
```

## ISM analysis - Did deviance change the structure perceived in the group?
...
#Run the Bayesian Hierarchical Infinite Similarity Model (ISM) using Julia
library(JuliaCall)
...
#use this code to test if the Julia function works as intended
library(Matrix)
...
for(i in 1:length(subs$SubjID)){
  ...
  #use all stimuli to create matrix size
  ...
  #need unique names of the stimuli used to label matrix
...
#Calculate average number of clusters and 95% CI
kplot = subs %>% 
...
#Run model for number of clusters
ismmod = subs %>% 
...
#Compute a test with k=2 as null
...
## Prediction Analysis - Did participants' predictions about a new agent's opinion change with deviance?
...
#Run two line test below and above the .5 deviance cuttoff
...
#Extract all meaningful metrics from the model
...
#Run model for confidence ratings (predictions)
...
#Compute averages by majority prediction
...
#Run two line tests below and above the .5 deviance cuttoff and by majority prediction
belowmod = predictions %>% 
  filter(Deviant_threshold <= .5) %>%
  group_by(pred_maj) %>% 
...
#Check to make sure deviant order randomization worked
data %>% 
...
Participants are demonstrating that they are learning about the deviant agent, but the degree of learning declines with subsequent tasks. This pattern could suggest that participant results reflect the first half of the structure learning model (up to the lowest point). Possible suggestion is to run 1B again with more chances to learn (i.e., more opinion issues) to see if the results show an increase accuracy in learning about the deviant over a longer period of tasks.
