---
title: "Subgrouping CogLoad V1 - Scratch Pad"
format: 
  dashboard:
    theme: darkly
---
...
#| eval: false
knitr::opts_knit$set(root.dir = '/Users/theoandre/Desktop/v2')
```
...
data = files %>%
  ...
  reduce(bind_rows)
data <- data %>% filter(matrix_cond == 'high')
#Preprocess
data = data %>% 
  group_by(SubjID) %>% 
...
data %>% 
  filter(passedattn == F) %>% 
  select(SubjID, age, race, gender, Deviant_threshold, matrix_cond, matrix_acc) %>% 
  distinct() %>% 
  table1(~age+ race+ gender+ matrix_acc | Deviant_threshold + matrix_cond, .)
```
...
data = files %>%
  map(~ read_csv(., show_col_types = F)) %>% 
  reduce(bind_rows)
data <- data %>% filter(matrix_cond == 'low')
#Preprocess
data = data %>% 
  group_by(SubjID) %>% 
  #pull out demographic responses into columns
