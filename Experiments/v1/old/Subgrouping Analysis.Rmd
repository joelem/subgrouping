...
learningtask$round = str_sub(learningtask$round, start = -1)
...
```{r, echo=F}
learningtask2 = data[data$part_of_expt == 'learning_task' & data$stim_deviance == 'deviant',]
learningtask2$round = sapply(learningtask2$dynamicVars_key, function(s) strsplit(s,'--')[[1]][2])
learningtask2$round = str_sub(learningtask2$round, start = -1)
learningtask2$corr = ifelse(learningtask2$response_correctness == 'correct', 1, 0)

learningperformance = ddply(learningtask2, .(SubjID, Deviant_threshold, round), summarize, accuracy = mean(corr)*100)
learningperformance.avg = ddply(learningperformance, .(Deviant_threshold, round), summarize, acc = mean(accuracy), n =length(accuracy), ci = qt(0.975,df=n-1)*sd(accuracy)/sqrt(n), l.ci= acc-ci, h.ci = acc+ci)
